...
/* Render the inline contract-authoring form (mirrors the extension form: per-year
   ...
   final year of a 2+ year deal). Rows are staged in addContractRows. */
function renderAddContractForm(box, startIdx){
  const startYr=ST.yearCols[startIdx];
  if(!startYr){box.textContent=""; addContractRows=[]; return;}
  // Seed (or re-seed) the first staged row when the form first appears or the
  // chosen start season changes.
  if(!addContractRows.length || ST.yearCols.indexOf(addContractRows[0].yr)!==startIdx){
    addContractRows=[{yr:startYr, unit:"dollar", value:NaN, isOpt:false}];
  }
  box.innerHTML="";
  ...
  addContractRows.forEach((er,ri)=>{
    ...
    // Only the final year of a 2+ year deal may be an option.
    const isLast = ri===addContractRows.length-1;
    if(isLast && addContractRows.length>1){
      const optBtn=document.createElement("button"); optBtn.className="rc-btn if-optbtn";
      const paint=()=>{ optBtn.textContent=er.isOpt?"✓ Final year is an option":"Make final year an option";
        optBtn.classList.toggle("on", er.isOpt); };
  ...
  // "Add another year" — append the next consecutive season.
  const lastYr=addContractRows[addContractRows.length-1].yr;
  const nextIdx=ST.yearCols.indexOf(lastYr)+1;
  const moreYr=ST.yearCols[nextIdx];
  if(moreYr){
    const more=document.createElement("div"); more.className="rc-addyear";
    more.innerHTML=`<span class="ay-plus">+</span><span>Add another year (${moreYr})</span>`;
    more.onclick=()=>{
      addContractRows[addContractRows.length-1].isOpt=false;  // only final year may be option
      addContractRows.push({yr:moreYr, unit:"dollar", value:NaN, isOpt:false});
      updateAddPlayerPreview(); };
...
function buildManualContract(name, age){
  ...
  addContractRows.forEach((er,ri)=>{
    const v = er.unit==="percent" ? capForYearM(er.yr)*er.value/100*1e6 : er.value*1e6;
    dict[er.yr]="$"+Math.round(v).toLocaleString("en-US");
    const isLast=ri===addContractRows.length-1;
    const asOption=isLast && addContractRows.length>1 && er.isOpt;
