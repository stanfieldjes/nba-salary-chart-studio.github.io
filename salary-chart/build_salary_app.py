...
// Σ(1+p)^i for i in 0..n-1 — the factor relating year-1 base salary to the total.
function _rampDenom(p,n){ let d=0; for(let i=0;i<n;i++) d+=Math.pow(1+p,i); return d; }
// Largest total ($M) that keeps year 1 at or below 35% of its season cap, given
// the chosen years and signed percent. Returns Infinity if no years.
function maxTotalForTerms(yrs, pct){
  if(!yrs||!yrs.length) return Infinity;
  const p=pct/100;
  return 0.35*capForYearM(yrs[0])*_rampDenom(p,yrs.length);
}
function splitContract(total, yrs, pct){
  if(!yrs||!yrs.length||isNaN(total)||total<0) return null;
  const p=pct/100, n=yrs.length;
  const base=(total*1e6)/_rampDenom(p,n);
  return {rows:yrs.map((yr,i)=>[yr, base*Math.pow(1+p,i)])};
}
/* buildTermsBlock — renders the shared control row (Years · Total · Change %) plus
   a read-only per-year preview, wired to a `state` object {total, years, pct,
   isOpt} and a yearsList(state.years)->[seasons] fn. `pct` is signed. The Total is
   capped: if the user enters more than maxTotalForTerms allows (year 1 would
   exceed 35% of cap), the value reverts to that maximum. Calls opts.onChange()
   after any change. Returns {el, preview()}. */
function buildTermsBlock(state, yearsForCount, opts){
  ...
  ctl.innerHTML=
    ...
    `<label class="mc-field"><span>Change</span>`+
  ...
  preHdr.textContent="Per-year preview (not editable)";
  ...
  function enforceTotalCap(){
    ...
    const cap=maxTotalForTerms(yrs, isNaN(state.pct)?0:state.pct);
    cappedNote=false;
  ...
  function preview(){
    preBox.innerHTML="";
    const res=splitContract(state.total, yearsForCount(state.years), isNaN(state.pct)?0:state.pct);
    ...
    res.rows.forEach(([yr,v],i)=>{
      ...
      r.innerHTML=`<span class="ifr-yr">${yr}</span>`+
  ...
  pctInp.oninput=()=>{ let p=parseFloat(pctInp.value); state.pct=isNaN(p)?0:Math.max(-8,Math.min(8,p));
    if(!isNaN(p)&&(p<-8||p>8)) pctInp.value=state.pct; fire(); };
...
/* ── Inline extension entry (rendered inside a player card) ──
   ...
   extension cap. */
let extState=null;            // {total, years, pct, dir, isOpt} or null when closed
...
function buildExtendForm(name){
  ...
  if(!extState){ extState={total:NaN, years:1, pct:0, isOpt:false}; }
  ...
  function pushExtPreview(){
    ...
    const res=splitContract(extState.total, extYearList(name, extState.years), extState.pct);
  ...
  apply.onclick=()=>{
    ...
    const p=extState.pct/100;
    for(let i=0;i<rows.length;i++){
      const [yr,v]=rows[i];
      const isFinal=i===rows.length-1;
      const label=`$${(v/1e6).toFixed(2)}M (${actualTotal.toFixed(1)}M total, ${p>=0?"+":""}${(p*100).toFixed(1)}%/yr)`;
...
function renderAddContractForm(box, startIdx){
  ...
  if(!addContract || addContract.startYr!==startYr){
    addContract={startYr, total:NaN, years:1, pct:0, isOpt:false};
  }
